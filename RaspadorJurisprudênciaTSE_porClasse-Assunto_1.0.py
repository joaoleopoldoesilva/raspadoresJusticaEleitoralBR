...
def raiz():
    raiz = "C:\\Users\\matheus\\Documents\\TEST\\13" #chamar como input
    return raiz
...
#siglasClasses = ("AC","AIME","AIJE","AP","AR","AI","AE","CRPP","CC","Cta","Cor","CZER","EE","Exc","EF","HC","HD","Inq","Inst","LT","MI","MS","PD","Pet","PC","PA","PP","Rcl","RCED","RE","RC","REspe","RHC","RHD","RMI","RMS","RO","RCand","RCF","ROPPF","RPP","Rp","RvC","RvE","SS")
...
def classeRelevante():
    listaClasses = []
    classeProcessual = input("Insira a classe processual que deseja incluir como parâmetro de busca, no formato de sigla (por exemplo, aije):").lower()
    while re.fullmatch(r'[A-Za-z]{1,6}', classeProcessual) == None:
        print("Por favor, utilize o fomato de sigla.")
        classeProcessual = input("Insira a classe processual que deseja incluir como parâmetro de busca, no formato de sigla (por exemplo, aije):").lower()
    listaClasses.append(classeProcessual)
    classeProcessual = input("Você pode incluir mais de 1 classe processual na sua busca, no formato de sigla (por exemplo, aije), se não quiser incluir mais classes, apenas tecle ENTER:").lower()
    while classeProcessual != "" and classeProcessual.upper() != "ENTER":
        if re.fullmatch(r'[A-Za-z]{1,6}', classeProcessual) == None:
            print("Por favor, utilize o fomato de sigla.")
            classeProcessual = input("Insira a classe processual que deseja incluir como parâmetro de busca, no formato de sigla (por exemplo, aije):").lower()
        else:
            listaClasses.append(classeProcessual)
            classeProcessual = input("Você pode incluir mais de 1 classe processual na sua busca, no formato de sigla (por exemplo, aije), se não quiser incluir mais classes, apenas tecle ENTER:").lower()
    robôLogger(3, "Classes processuais:", str(listaClasses).strip('[]')+'.\n')
    print(listaClasses)
    return listaClasses

def assuntoRelevante(): #jogar o tratamento do nome para o criaURL
    listaAssuntos = []
    assunto = input("Insira o assunto que deseja incluir como parâmetro de busca:").lower()
    listaAssuntos.append(assunto)
    assunto = input("Você pode incluir mais de 1 assunto na sua busca, se não quiser incluir mais assuntos, apenas tecle ENTER:").lower()
    while assunto != "" and assunto.upper() != "ENTER":
        listaAssuntos.append(assunto)
        assunto = input("Você pode incluir mais de 1 assunto na sua busca, se não quiser incluir mais assuntos, apenas tecle ENTER:").lower()
    robôLogger(3, "Assuntos:", str(listaAssuntos).strip('[]')+'.\n')
    print(listaAssuntos)
    return listaAssuntos

def criaURLinicial(dataInicio, dataFim, listaTribunais, siglaLei='', descriçãoLei='', numLei='', anoLei='', classe='', assunto=''): ## falta acrescentar classe e assunto
    stringTribunais = str()
...
def main(dataInicio, dataFim, listaTribunais, listaClasses, listaAssuntos):
    nClasse, nAssunto = 0, 0
    if 'TSE' in listaTribunais:
        w = 2
    else:
        w = 1
    nBusca = len(listaClasses)*len(listaAssuntos)
    print("iniciando %s buscas..." % nBusca)
    for i in range(nBusca):
        print("%s..." % str(i+1))
        nTribunal = 0
        robôLogger(5, listaClasses[nClasse].upper(), listaAssuntos[nAssunto])
        driver.get(criaURLinicial(dataInicio, dataFim, listaTribunais, classe=listaClasses[nClasse], assunto=listaAssuntos[nAssunto]))
        driver.find_element_by_name("submit").click()
        time.sleep(90)  ## como fazer uma espera mais inteligente? ...até não achar "Pesquisando..."
        ...
        for tr in trs[w:]:
            try:
                ...
                print("Erro percebido, StaleElementReferenceException. Na passagem '%s', classe '%s' assunto '%s', tribunal '%s'. Linha 303." % (str(i+1), listaClasses[nClasse], listaAssuntos[nAssunto], listaTribunais[nTribunal]))
            ...
            try:
                ...
                time.sleep(2)
                set_string_preferce("browser.download.dir", criaPath(listaTribunais[nTribunal], classe=listaClasses[nClasse], assunto=listaAssuntos[nAssunto])) #listaReferênciasLegislativas[nRefLeg], artigosReferênciaLegislativa[nRefLeg][nArtLeg])) ##acrescentar classe e assunto
                driver.switch_to.window(window_docs)
                ha_prox_pag = True
                j=0
                while ha_prox_pag:
                    ...
                    for caixa in caixas:
                        ...
                        try:
                            ...
                            print("Erro percebido, StaleElementReferenceException. Na passagem '%s', classe '%s' assunto '%s', tribunal '%s'. Linha 336." % (str(i+1), listaClasses[nClasse], listaAssuntos[nAssunto], listaTribunais[nTribunal]))
                            return
                        InteiroTeor.click()
                ...
                robôLogger(2, texto_td0, texto_td1)
                driver.close()
                driver.switch_to.window(driver.window_handles[0])   ## precisa dar tempo?
            nTribunal+=1
        driver.find_element_by_link_text("Voltar").click()
        driver.find_element_by_name("livre").clear()
        if nAssunto != len(listaAssuntos)-1:
            nAssunto+=1
        else:
            nClasse+=1
            nAssunto=0
...
if __name__ == '__main__':
    ...
    listaClasses = classeRelevante()
    listaAssuntos = assuntoRelevante()
#    listaReferênciasLegislativas, artigosReferênciaLegislativa = legislaçãoRelevante()
    ...
    main(dataInicio, dataFim, listaTribunais, listaClasses, listaAssuntos) #listaReferênciasLegislativas, artigosReferênciaLegislativa)
    with open(loggerPath(), 'r') as f:
        print(f.read())
